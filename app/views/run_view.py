...
def build_confirm_dialog(mode, body_text, est_text, on_confirm, on_cancel,
                         start_label="이수 시작"):
    """되돌릴 수 없는 영상 이수(형성평가 자동 제출) 확인 다이얼로그.

    체크박스에 동의하기 전에는 시작 버튼이 **로직 차원에서** 막힌다
    (UI disabled 뿐 아니라 핸들러에서도 차단 → 실수 클릭·자동화로도 제출 안 됨).
    start_label 로 버튼 문구를 바꿀 수 있다(예약 등록 등 재사용).
    반환: (dlg, agree_checkbox, start_btn) — page 없이도 오프라인 단위테스트 가능.
    """
    agree = ft.Checkbox(label="형성평가 답안이 실제로 제출됨을 이해합니다",
                        value=False)
    ...
    dlg = ft.AlertDialog(
        modal=True,
        title=ft.Text(f"⚠️ '{mode}' — 형성평가 자동 제출 확인"),
    ...
    mode_group = ft.RadioGroup(
        ...
        content=ft.Column([
            ...
            ft.Radio(value=MODE_WATCH,
                     label="영상 이수 (⚠️ 형성평가 자동 제출 · 노트 없음)"),
