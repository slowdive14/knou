...
`paused=True` 인 채 위치가 나가지 않거나, 몇 초 가다 멈추고 그 뒤로는 프레임
상태조차 못 읽는다(`on_progress` 가 불리지 않음). 시도해 본 것:

- 재생 전에 다른 영상 정지 → 0→45초까지는 나아갔으나 다시 멈춤
- 매 폴링마다 다른 영상 정지 + 대상 재생 강제(`solo_guard`) → 8초에서 멈춤

Kollus 가 한 페이지에서 한 영상만 돌리는 듯하나, 그것만으로는 설명되지 않는다
(대상만 남겨도 멈춘다). **화면을 직접 보며 더 살펴야 한다.**
