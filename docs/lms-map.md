...
### 11-4. 진도 적립 메커니즘 — 실측 확정 (1주차, 2026-09-15) ✅✅

**2배속 자동 이수가 된다.** 3분 재생으로 1주차 진도가 **0.52% → 5.67%** 로
올랐고, 배속 때문에 깎이지 않았다.

- **화면 진입 순서가 중요하다**: 대시보드 → `moveClassRoom` → 강의 URL.
  바로 강의 URL 로 가면 Kollus iframe 이 **채워지지 않는다**(실측: 프레임 1개,
  `<video>` 0개). 세션에 학기·과목 컨텍스트가 잡혀야 한다.
- **`<video>` 에 직접 닿는다** — Kollus iframe 이 막지 않는다. 전자캠퍼스와
  같은 방식으로 `playbackRate` 와 `play()` 를 쓸 수 있다.
- ⚠️ **배속은 폴링마다 다시 걸어야 한다.** 한 번만 설정하면 Kollus 가 1 로
  되돌린다(실측: 15초 뒤 rate=1). 다시 걸어 주면 15초마다 +30초로 유지된다.
- **보고 경로**: `POST /lctr/kollusCallback.do` (본문 `json_data=<URL 인코딩 JSON>`).
  주기는 **~173초**, 그리고 재생 시작·정지 시점.
- **적립 기준 = 블록 커버리지**. 영상을 **30개 블록**으로 쪼개 블록별 시청률을 보낸다:

      content_info: duration=3727, playtime=302, playtime_percent=8,
                    real_playtime=302, runtime=172, showtime=155,
                    last_play_at=304, serial=1
      block_info:   block_count=30,
                    b0=1 t0=123 p0=98    (블록0: 완료, 123초, 98%)
                    b1=1 t1=125 p1=100
                    b2=1 t2=54  p2=43
                    b3..b29 = 0

  `b*`=블록 완료 여부, `t*`=그 블록에서 본 시간, `p*`=시청 비율(%).
- ⚠️ **`runtime`(벽시계 172초)과 `playtime`(영상 위치 302초)을 둘 다 보낸다** —
  서버가 배속을 알 수 있다는 뜻이다. 2배속까지는 깎이지 않는 것을 확인했으나,
  더 높은 배속은 검증하지 않았다. **2배를 넘기지 말 것.**
- **한 주차에 영상이 여러 개**(1주차: 407초 + 3727초). 동시에 재생되지 않으므로
  (하나를 켜면 다른 하나는 `paused=True`) **순차로** 돌려야 한다.
- 진도 확인: `check_week_progress.py`(읽기 전용).

### 11-5. 자동이수 구현 — 1주차 실측 (2026-09-15) ✅🔶

`knouon.py` + `watch_knouon_one.py`. **본강의는 완전히 작동한다.**

| 항목 | 결과 |
|---|---|
| 본강의(3727초) | **100% 완청** · 실제 47분(2배속) · `status=ended` |
| 1주차 진도 | 0.52% → **69.65%** |
| 오리엔테이션(407초) | ❌ 몇 초 만에 멈춘다(아래) |

구현하며 부딪힌 것(모두 코드에 반영·주석):

1. **길이는 재생을 시작한 뒤에 읽는다.** Kollus 가 메타데이터를 늦게 채워
   `clip_inventory` 시점에는 `dur=0` 이다. 그대로 예산을 잡으면 407초짜리가
   예산 60초로 잡혀 108초에 '시간 초과'가 된다 → `clip_duration()`.
2. **예산 여유배수를 3.0 으로.** 배속을 2 로 걸어도 실제로는 그만큼 안 나올
   때가 있다(실측 구간: 15초에 12초 진행 ≈ 0.8배). progressive MP4 라 버퍼가
   못 따라간다. 뒤에 회복되어 다시 2배속이 되었다 → `WAIT_FACTOR`.
3. **강의 화면은 `domcontentloaded` 로 기다리면 타임아웃 난다.** Kollus iframe 이
   계속 물고 있다. `commit` 만 확인하고 재생 프레임이 차오르길 따로 기다린다
   → `wait_for_players()`.
4. **저장은 일시정지로 유도한다.** 전자캠퍼스의 `fnPlayStop()` 은 그쪽 전용
   함수다 → `trigger_save()`.
5. `watch.py` 의 `_clip_frames` 가 Kollus 프레임도 알아보게 넓혔다. 그 덕에
   `_play_until_end`·`clip_inventory`·`_reapply_speed` 를 두 시스템이 함께 쓴다.

**진도 배분이 길이 비례가 아니다** — 본강의(3727초) 100%가 약 46%p, 오리엔테이션
(407초) 12%만 봐도 5.89%p 올랐다. 두 영상이 비슷한 비중을 갖는 것으로 보이므로
**오리엔테이션도 반드시 봐야** 100%가 된다.

#### 🔑 오리엔테이션이 멈추던 원인 — **배속을 계속 다시 걸어서** (해결)

화면을 띄워 놓고 진단(`probe_orient.py`)해 보니 오리엔테이션은 **아무 문제 없이
재생되고 있었다**. 120초 동안 10초마다 정확히 +10초, `오류=None`,
`받음=충분히받음`. 다만 **배속이 1.0x** 였다.

Kollus 는 영상에 따라 배속을 **거부하고 1.0x 로 되돌린다**(본강의는 2배속이
걸리고, 오리엔테이션은 안 걸린다). 그런데 `watch._play_until_end` 는 배속이
요청보다 낮으면 **폴링마다 다시 건다**(`_reapply_speed`). 그 반복이 재생을
끊고 있었다 — 우리가 고치려던 것이 원인이었다.

대응: `effective_speed()` 로 **실제로 걸린 배속을 확인해 받아들이고**, 그 값으로
예산을 잡아 `_play_until_end` 에 넘긴다. 그러면 재설정이 일어나지 않는다.
폴링마다 재생을 강제하던 `solo_guard` 도 쓰지 않는다(그것이 오히려 8초 만에
끊었다). **재생 시작 때 다른 영상을 한 번 멈추는 것으로 충분하다.**

#### ✅ 1주차 이수 완료 (2026-09-15)

    오리엔테이션(407초) 1배속 → 100% 완청 · 실제 7분 38초
    1주차 진도 70.51% → **100.0%** · 완료판정 True

전체 경로: 0.52% → 100%. 본강의는 2배속(47분), 오리엔테이션은 1배속(8분).
**이수 완료 판정은 진도율 100%** 에서 선다(`COMPLETE_PERCENT=95` 로 여유를 뒀다).

### 11-6. 파이프라인 통합 (2026-09-15) ✅

- **목록**: `main._run` 과 `snapshot.refresh_snapshot` 두 곳에서, 전자캠퍼스가
  차시를 못 주는 과목이면 knouon 을 본다. 스냅샷을 빠뜨리면 앱 목록에
  '차시 0개'로 남아 고를 수조차 없다.
- **watch**: `_stage_watch` 가 knouon 과목이면 `knouon.watch_week` 로 간다.
- **나머지 단계**(exam·download·capture·extra): 아직 손대지 않았다. 실패가
  아니라 **'건너뜀'**으로 남긴다(`_knouon_unsupported`) — 실패로 기록하면
  의존하는 뒤 단계까지 막힌다.
- 실측: `바이오통계학: knouon 에서 15주차 확인` → 주차 선택 → 재생까지 흐른다.

#### ⚠️ 배속은 붙는 대로 받아들일 것

배속을 **다시 거는 순간 플레이어가 깨진다.** 실측한 세 경우:

| 영상 | 배속 | 결과 |
|---|---|---|
| 1주차 본강의 3727초 | 2.0 | 완주(47분) |
| 1주차 오리엔테이션 407초 | 1.0 고정 | 2배속을 계속 걸었더니 8초에서 멈춤 → 받아들이니 완주(8분) |
| 2주차 본강의 3530초 | 1.0 고정 | 2배속을 다시 걸었더니 붙기는 했으나 `<video>` 가 사라짐 → 받아들여 1배속 59분 |

`effective_speed()` 는 **읽기만** 한다. `start_clip` 에서 한 번 건 것으로 끝내고,
관찰(8회×5초)해서 붙은 값을 그대로 쓴다. 느려도 완주하는 쪽이 낫다.

### 11-7. 아직 모르는 것 ❓

- 이수 완료 판정 기준(진도율 몇 %에서 '완료'가 되는가, 어디서 읽는가).
  `/lrnsts/selectLrnStsClassDetailView.do` 로 보인다.
- 강의자료실(`bbsTycd=DATARM`)의 주차별 강의록 유무
- 돌발퀴즈 제출 방식(조회 엔드포인트만 확인했다)
