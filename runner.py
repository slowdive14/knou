...
# ---------------------------------------------------------------------------
# Phase 3: 되돌릴 수 없는 이수(형성평가 제출) 안전장치 (순수)
# ---------------------------------------------------------------------------
# 영상 이수 단계(watch)는 형성평가 답안을 실제 서버에 제출한다 → 되돌릴 수 없음.
_IRREVERSIBLE_MODES = {"이수", "전체"}


def requires_confirm(mode: str) -> bool:
    """해당 모드가 되돌릴 수 없는 제출(형성평가)을 포함하면 True."""
    return mode in _IRREVERSIBLE_MODES


def confirm_message(mode: str) -> str:
    """이수 실행 전 보여줄 경고문(형성평가 자동 제출의 비가역성 명시)."""
    return (
        f"'{mode}' 모드는 영상 이수와 함께 형성평가(연습문제) 답안이 "
        "실제 서버에 제출됩니다.\n"
        "⚠️ 이 제출은 되돌릴 수 없습니다. 본인 계정·예습 목적임을 "
        "이해하고 동의하는 경우에만 진행하세요."
    )
