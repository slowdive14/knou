...
# ---------------------------------------------------------------------------
# Phase 3: 실제 서버 이수/제출 안전장치 (순수)
# ---------------------------------------------------------------------------
# 이수/전체 모드는 실제 방송대 서버에 영상 시청(이수) 기록을 남기고, 영상 중
# 돌발퀴즈와 형성평가(연습문제) 답안을 자동 제출한다 → 본인 계정에 반영되며
# 되돌릴 수 없으므로 사전 동의가 필요하다.
_IRREVERSIBLE_MODES = {"이수", "전체"}


def requires_confirm(mode: str) -> bool:
    """해당 모드가 실제 서버에 반영되는 이수·제출 동작을 포함하면 True."""
    return mode in _IRREVERSIBLE_MODES


def confirm_message(mode: str) -> str:
    """이수 실행 전 보여줄 경고문(실제 서버 이수 + 형성평가 제출 명시)."""
    return (
        f"'{mode}' 모드는 실제 방송대 서버에 영상 시청(이수) 기록을 남기고, "
        "영상 중 돌발퀴즈와 형성평가(연습문제) 답안을 자동으로 제출합니다.\n"
        "⚠️ 실제 본인 계정에 반영되며 되돌릴 수 없습니다. "
        "본인 계정·학습 목적임을 이해하고 동의하는 경우에만 진행하세요."
    )
...
def watch_sleep_warning() -> str:
    ...
    실행 중 **유휴 절전**은 앱이 직접 막는다(main.run 이 억제를 건다). 다만
    노트북 뚜껑을 닫는 것은 사람이 지시한 절전이라 프로그램이 막을 수 없어,
    그 부분만 사용자에게 알린다(실측: 뚜껑을 닫은 사이 3시간 넘게 잠들어
    80분짜리 강의 하나가 4시간 걸렸다).
    """
    return ("⚠️ 실행 중 유휴 절전은 앱이 막아 줍니다(모니터만 꺼지는 것도 "
            "괜찮습니다). 다만 노트북 뚜껑을 닫으면 그대로 잠들어 이수가 "
            "멈추니, 뚜껑은 열어 두세요.")
